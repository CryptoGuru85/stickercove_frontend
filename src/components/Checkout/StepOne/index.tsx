...
export const StepOne = ({ onChangeHandler, validated, onContinue, onBack }) => {
  const checkoutValue = useSelector(state => state.checkout)
  const orderList = useSelector(state => state.abandon.orderList)
  ...
  const handleSelect = async (address) => {
    ...
    await getGeocode({address: `${address} usa`})
      .then((results) => {
    ...
    if (addressType != 'street_address' && addressType != 'premise') {
      setValue("", false)
      alert("The address must be of a delivery location of a place.")
      return;
    }
    ...
    console.log("I'm changing shipping-zip from aftter GEO")
  ...
  const submit = () => {
    ...
    let userInfo = {
      ...
      streetAddress: checkoutValue.shippingStreetOne,
      shippingCity: checkoutValue.shippingCity,
      shippingStreet: checkoutValue.shippingStreetOne,
      shippingStreetTwo: checkoutValue.shippingStreetTwo,
      shippingZip: checkoutValue.shippingZip,
      shippingState: checkoutValue.shippingState,
      cart: orderList,
  ...
  return (
    <>
      ...
      <input
        ...
        className="checkout-panel-info-input"
        value={checkoutValue.shippingStreetTwo}
        onChange={onChangeHandler}
      />
