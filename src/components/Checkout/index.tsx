...
import { APP_SECRETS } from "src/local-secrets";
...
export const Checkout = ({
  ...
  const stripePromise = loadStripe(APP_SECRETS.STRIPE_PKEY_LIVE);
  ...
  const onContinue = () => {
    if (stage == 2) return;
    if (validateStage1()) setStage(stage + 1);
  };
  const onBack = () => {
    if (stage == 0) {
      router.back();
    ...
    orderList.map((order, index) => (
      <div key={index} className="checkout-panel-status-box-item">
        ...
        <div className="text">
          ...
          <span className="quantity">Pending Proof Approval</span>
        </div>
